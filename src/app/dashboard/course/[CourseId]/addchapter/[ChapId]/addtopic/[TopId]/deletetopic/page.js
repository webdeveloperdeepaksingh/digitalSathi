...
export default function DelTopic({params}) {

  const router = useRouter();

  const handleSubmit = async (e) => {   
    e.preventDefault();
    let response = await fetch(`http://localhost:3000/api/courses/${params.CourseId}/chapter/${params.ChapId}/topic/${params.TopId}`, {
        method:'DELETE'
    });
