...
export default function AddTopic({params}) {
  ...
  useEffect(() =>{
    async function fetchTopList(){
      const response = await fetch(`http://localhost:3000/api/courses/${params.CourseId}/chapter/${params.ChapId}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try
    {        
    const result = await fetch (`http://localhost:3000/api/courses/${params.CourseId}/chapter/${_id}/topic`, 
    {
        method:'POST',
