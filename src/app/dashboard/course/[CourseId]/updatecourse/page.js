...
export default function UpdateCourse({params}) {
    ...
    useEffect(() =>{
        ...
        setEditorContent(course.result.coDesc)
        setImage(`/images/${course.result.coImage}`) 
        }
    fetchData();
    },[params.CourseId]);
    ...
    const handleImage = (e) => {
        ...
        e.preventDefault();
            ...
            data.coImage=`coImage_${params.CourseId}.${imageData.name.split('.').pop()}`;
    ...
    try
    {
      ...
      {
        method:'PUT',
        body:JSON.stringify({coName: data.coName, coTags:data.coTags, coIntro:data.coIntro, coDesc:editorContent, coPrice:data.coPrice, coDisc:data.coDisc, coVal:data.coVal, coCat:data.coCat, coImage:data.coImage}),
      });
