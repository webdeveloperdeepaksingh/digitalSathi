...
export default function DelCategory({params}) {

  const router = useRouter();

  const handleSubmit = async (e) => {   
    e.preventDefault();
    let response = await fetch(`http://localhost:3000/api/categories/${params.CatId}`, {
        method:'DELETE'
    });
            ...
            <div className='flex flex-col bg-white p-4 rounded-lg'>
                <form className='flex flex-col p-3'onSubmit={handleSubmit}>
                    ...
                    <div className='flex gap-3 justify-center'>
                        ...
                        <Link href='/dashboard/categorylist' className='py-1 px-2 rounded-sm bg-gray-600 hover:bg-gray-500 text-white text-sm font-bold'>CANCEL</Link>
                    </div>
                </form>
            </div>
        </div>
