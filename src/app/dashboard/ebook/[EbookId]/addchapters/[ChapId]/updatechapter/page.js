...
export default function UpdateEbookChap({params}) {
  ...
  useEffect(() =>{
    async function fetchData() {
      let chapData = await fetch(`http://localhost:3000/api/ebooks/${params.EbookId}/addchapter/${params.ChapId}/updatechapter`);
  ...
  const handleFileUpload = async (e) =>{
    e.preventDefault();
     ...
     data.pdfFile =`ebkPdf_${params.ChapId}.${fileData.name.split('.').pop()}`;
  ...
  try
  {
    ...
    const result = await fetch (`http://localhost:3000/api/ebooks/${params.EbookId}/addchapter/${params.ChapId}/updatechapter`, 
    {
      method:'PUT',
