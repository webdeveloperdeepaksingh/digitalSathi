...
export default function AllowEvent({params}) {
    ...
    const [eventData, setEventData] = useState({allowAccess:[]}); 
    ...
    useEffect(() =>{
        ...
        try 
            {
                ...
                if(userEmailId.transectionList){
                    userForEvent = userEmailId.transectionList.filter((item) => {
                        return item.custProducts.some((product) => product.prodType === "events" && product.prodId === params.EventId );
                    });
                }
                else{
                    userForEvent = userEmailId.filter((item) => {
                        return item.custProducts.some((product) => product.prodType === "events" && product.prodId === params.EventId );
                    });
                }
                const filteredUsers = userForEvent.filter((user) => {            
                    const hasAccess = eventData.allowAccess.some((access) => access.usrEmailId === user.custEmail);
                    return !hasAccess;
                    });
                    console.log()
        ...
        const handleSubmit = async (e) => {
            ...
            if (!data.shareAccess?.trim()) {
                errMsg.push('Please choose access.');    
            }
    ...
    <div className='flex h-auto my-1 justify-center max-w-[460px] mx-auto rounded-xl shadow-lg'>
        <form className='flex p-9 rounded-lg flex-col w-auto bg-gray-100 h-auto border-2 border-amber-500' onSubmit={handleSubmit}>
            ...
            <div className='flex flex-col mb-3'>
                ...
                <input type='text' name='courseId' value={data?._id} className='py-2 px-3 mt-2 focus:outline-amber-500'></input>
            </div>
